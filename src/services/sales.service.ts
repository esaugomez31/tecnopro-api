...
import { productsGetByIds, branchGetById, customerGetById } from "."
...
export const saleDetailCreate = async (
...
export const getSaleCreated = async (
  idSale: number,
  uuid: string,
): Promise<ISale | null> => {
  try {
...
const generateSaleData = async (
  ...
  dbproducts.forEach((dbProd) => {
    ...
    productInv.forEach((prod) => {
      ...
      const currProfit = parseFloat(
        (affectSale - (purchasePrice * quantity + currVat)).toFixed(4),
      )
      const currUserCommision = parseFloat(
        (currProfit * (userCommissionP / 100)).toFixed(4),
      )
      ...
      totals.usersCommission = parseFloat(
        (totals.usersCommission + currUserCommision).toFixed(2),
      )
...
export const saleUpdateStatus = async (
  idSale: number,
  status: boolean,
): Promise<ISale> => {
  try {
...
export const saleGetAll = async (
  filterParams: ISaleFilters,
  settings: IFilterSettings,
): Promise<IGetSalesResponse> => {
  try {
...
export const saleGetById = async (
  idSale: number,
  settings?: IFilterSettings,
): Promise<IGetSaleByIdResponse> => {
  try {
    const relations =
      settings?.include !== undefined ? getSaleIncludeFields(settings.include) : []
...
const existValuesValidations = async (
  ...
  const [existBranch, existCustomer] = await Promise.all([
    branchGetById(idBranch),
    customerGetById(idCustomer),
  ])
