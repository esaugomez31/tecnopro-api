...
const validSortFields = ['idSale', 'total', 'vat', 'totalProfit', 'grossProfit', 'usersCommission', 'refunded']
...
const saleCommonValidations = (optional = false): any => [
  ...
  body('dteOperationCondition')
    .optional()
    .isIn(validDteOperationCondition).withMessage(`dteOperationCondition must be one of the following: ${validDteOperationCondition.join(', ')}.`)
    .customSanitizer(value => value as string)
]

export const validateSaleCreation = (): any => {
  return [
    ...saleCommonValidations(),
...
export const validateSaleUpdateStatus = (): any => {
  return [
    param('idSale')
      .isInt().withMessage('idSale must be an integer')
...
export const validateGetSales = (): any => {
  return [
    ...
    query('idUser')
      .optional()
      .isInt().withMessage('idUser must be a integer')
      .customSanitizer(Number),

    query('idCustomer')
      .optional()
      .isInt().withMessage('idCustomer must be a integer')
      .customSanitizer(Number),

    query('idBranch')
      .optional()
      .isInt().withMessage('idBranch must be a integer')
      .customSanitizer(Number),

    query('startDate')
      .optional()
      .isISO8601().withMessage('startDate must be a valid date in ISO8601 format')
      .toDate(),

    query('endDate')
      .optional()
      .isISO8601().withMessage('endDate must be a valid date in ISO8601 format')
      .toDate(),
