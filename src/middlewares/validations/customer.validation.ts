...
const validSortFields = ['idCustomer', 'name', 'phoneNumbers', 'email', 'idCountry', 'idDepartment', 'idMunicipality']

const customerCommonValidations = (optional = false): any => [
  body('name')
    .optional(optional)
    .isString().withMessage('name must be a string')
    .isLength({ min: 1, max: 75 }).withMessage('name must be between 1 and 75 characters long'),

  body('tradeName')
    .optional({ checkFalsy: true })
    .isString().withMessage('tradeName must be a string')
    .isLength({ max: 150 }).withMessage('tradeName must have a maximum of 150 characters'),

  body('dui')
    .optional({ checkFalsy: true })
    .isString().withMessage('dui must be a string')
    .isLength({ min: 9, max: 30 }).withMessage('dui must be between 9 and 30 characters long')
    .matches(/^\d+$/).withMessage('dui must contain only numbers'),

  body('nit')
    .optional({ checkFalsy: true })
    .isString().withMessage('nit must be a string')
    .isLength({ min: 9, max: 14 }).withMessage('nit must be between 9 and 14 characters long')
    .matches(/^\d+$/).withMessage('nit must contain only numbers'),

  body('nrc')
    .optional({ checkFalsy: true })
    .isString().withMessage('nrc must be a string')
    .isLength({ min: 2, max: 8 }).withMessage('nrc must be between 2 and 8 characters long')
    .matches(/^\d+$/).withMessage('nrc must contain only numbers'),

  body('phoneNumbers')
    .optional({ checkFalsy: true })
    .isString().withMessage('phoneNumbers must be a string')
    .isLength({ max: 30 }).withMessage('phoneNumbers length does not exceed 30 characters')
    .matches(/^(\d{8,30})(,\d{8,30}){0,2}$/) // allow three phone numbers, separates by commas
    .withMessage('phoneNumbers must be between 8 and 30 digits long and can contain up to three numbers separated by commas'),
  ...
  body('address')
    ...
    .isLength({ min: 2, max: 200 }).withMessage('address must be between 1 and 200 characters long'),
...
export const validateCustomerUpdate = (): any => {
  return [
    param('idCustomer')
      .isInt().withMessage('idCustomer must be an integer')
