...
const branchCommonValidations = (optional = false): ValidationChain[] => [
  ...
  body("email")
    ...
    .isLength({ max: 100 })
    .withMessage("dteSenderEmail length does not exceed 100 characters"),
  ...
  body("vatEnabled")
    .optional()
    .isBoolean()
    .withMessage("vatEnabled must be a boolean")
  ...
  body("dte").optional({ checkFalsy: true }),

  body("dte.dteActive")
    .optional()
    .isBoolean()
    .withMessage("dteActive must be a boolean")
    .customSanitizer(stringToBoolean),

  body("dte.dteEnvironment")
    .optional()
    .isIn(validDteEnvironment)
    .withMessage(
      `dteEnvironment must be one of the following: ${validDteEnvironment.join(", ")}.`,
    )
  ...
  body("dte.dteApiJwt")
    .optional({ checkFalsy: true })
    .isString()
    .withMessage("dteApiJwt must be a string")
    .isLength({ min: 1, max: 255 })
    .withMessage("dteApiJwt must be between 1 and 255 characters long"),

  body("dte.dteApiJwtDate")
    .optional()
    .isISO8601()
    .withMessage("dteApiJwtDate must be in ISO 8601 format"),

  body("dte.dteSenderNit")
    .optional({ checkFalsy: true })
    .isString()
    .withMessage("dteSenderNit must be a string")
    .isLength({ min: 9, max: 14 })
    .withMessage("dteSenderNit must be between 9 and 14 characters long")
    .matches(/^\d+$/)
    .withMessage("dteSenderNit must contain only numbers"),

  body("dte.dteSenderNrc")
    .optional({ checkFalsy: true })
    .isString()
    .withMessage("dteSenderNrc must be a string")
    .isLength({ min: 2, max: 8 })
    .withMessage("dteSenderNrc must be between 2 and 8 characters long")
    .matches(/^\d+$/)
    .withMessage("dteSenderNrc must contain only numbers"),

  body("dte.dteSenderEmail")
    .optional({ checkFalsy: true })
    .isEmail()
    .withMessage("dteSenderEmail must be a valid email")
    .isLength({ max: 100 })
    .withMessage("dteSenderEmail length does not exceed 100 characters"),
  ...
  body("dte.dteSenderPhone")
    .optional({ checkFalsy: true })
    .isString()
    .withMessage("dteSenderPhone must be a string")
    .isLength({ max: 30 })
    .withMessage("dteSenderPhone length does not exceed 30 characters")
    .matches(/^(\d{8,30})(,\d{8,30}){0,2}$/) // allow three phone numbers, separates by commas
    .withMessage(
      "dteSenderPhone must be between 8 and 30 digits long and can contain up to three numbers separated by commas",
    ),

  body("dte.dteActivityCode")
    .optional()
    .isString()
    .withMessage("dteActivityCode must be a string")
    .isLength({ min: 5, max: 6 })
    .withMessage("dteActivityCode must be between 5 and 6 characters long"),

  body("dte.dteActivityDesc")
    .optional({ checkFalsy: true })
    .isString()
    .withMessage("dteActivityDesc must be a string")
    .isLength({ min: 5, max: 150 })
    .withMessage("dteActivityDesc must be between 5 and 150 characters long"),

  body("dte.dteSenderName")
    .optional({ checkFalsy: true })
    .isString()
    .withMessage("dteSenderName must be a string")
    .isLength({ min: 1, max: 250 })
    .withMessage("dteSenderName must be between 1 and 250 characters long"),

  body("dte.dteSenderTradeName")
    .optional({ checkFalsy: true })
    .isString()
    .withMessage("dteSenderTradeName must be a string")
    .isLength({ min: 1, max: 150 })
    .withMessage("dteSenderTradeName must be between 1 and 150 characters long"),

  body("dte.dteEstablishment")
    .optional()
    .isIn(validDteEstablishment)
    .withMessage(
      `dteEstablishment must be one of the following: ${validDteEstablishment.join(", ")}.`,
    )
    .customSanitizer((value) => value as string),
