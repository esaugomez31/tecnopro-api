...
const productCommonValidations = (optional = false): any => [
  ...
  body('barcodeGenerated')
    .optional()
    .isBoolean().withMessage('barcodeGenerated must be a boolean')
  ...
  body('purchasePrice')
    .optional()
    .isDecimal({ decimal_digits: '0,4' }).withMessage('purchasePrice must be a decimal with up to 4 decimal places')
    .custom(value => value > 0).withMessage('purchasePrice must be greater than 0'),

  body('purchasedBy')
    .optional()
    .isIn(purchasedBy).withMessage(`orderBy must be one of the following: ${purchasedBy.join(', ')}.`)
  ...
  body('stock')
    .optional(optional)
    .isDecimal({ decimal_digits: '0,4' }).withMessage('stock must be a decimal with up to 4 decimal places')
  ...
  body('idBranch')
    .optional(optional)
    .isInt({ min: 1, max: 99999999999 }).withMessage('idBranch must be an integer')
    .customSanitizer(Number),
...
export const validateProductUpdate = (): any => {
  return [
    param('idProduct')
      .isInt().withMessage('idProduct must be an integer')
      .customSanitizer(Number),
